...
FrequencyPlot::FrequencyPlot(QCustomPlot *parent) noexcept
    : buffer_(kBufferSize, 0.0), processedData_(kBufferSize / 2, kMinDB),
      fft_ip(2 + static_cast<int>(std::sqrt(kBufferSize / 2)), 0), fft_w(kBufferSize / 2, 0.0),
      fft_window(kBufferSize, 0.0), frequency_axis(kBufferSize / 2, 0.0)
{
    plot_ = parent;
...
{
    std::ranges::generate(
        fft_window, [i = 0, this]() mutable { return 0.5 * (1.0 - std::cos(2.0 * M_PI * i++ / (kBufferSize - 1))); });
